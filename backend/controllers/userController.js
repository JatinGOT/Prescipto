...
const registerUser = async (req, res) => {
    try {
        ...
        if (!validator.isEmail(email)) {
            return res.json({ succuss: false, message: "enter a valid email" })
        }
        // validating strong password
        if (password.length < 8) {
            return res.json({ succuss: false, message: "enter a strong password " })
        }
...
const updateProfile = async (req, res) => {
    try {
        const { userId, name, phone, address, dob, gender, imageFile } = req.body;
        ...
        // If an image file is provided in Base64 format
        if (imageFile) {
            // Upload image to Cloudinary
